...
-- How many unique customers and categories we have ?
...
WITH average_cte as 
(SELECT substr(sale_date,1,7) as month_year, round(avg(total_sale),2) as avg_sales FROM retail_sales GROUP BY substr(sale_date,1,7))
, rank_cte AS 
(SELECT month_year, avg_sales, RANK() OVER (PARTITION BY substr(month_year,1,4) ORDER BY avg_sales desc) as rank_data FROM average_cte)
